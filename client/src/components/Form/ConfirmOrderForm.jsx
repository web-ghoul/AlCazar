...
const ConfirmOrderForm = ({ loading }) => {
    ...
    return (
      <>
        <Title title={"Confirm Order"} h={"h4"} align={"center"} fw={"600"} />
        <Paper className={`grid  jcs aic g20 pad20`}>
          <Box className={`flex jcfs aic g10`}>
            <CheckCircleRounded
              sx={{ color: (theme) => theme.palette.whatsapp }}
            />
              ...
              <Box key={i} className={`flex jcsb aic g20`}>
                <Box className={`flex jcfs aic g10`}>
                  <Box
                    ...
                    sx={{ backgroundImage: `url(${data.data.images[0]})` }}
                  />
                  <Box className={`grid jcfs aic g10`}>
                    ...
                    <Typography variant="h6">
                      L{data.data.dimensions[data.dimension || 0].length} x W
                      {data.data.dimensions[data.dimension || 0].width} x H
                      {data.data.dimensions[data.dimension || 0].height}
                    </Typography>
                  </Box>
                </Box>
          ...
          <Box className={`grid jcs aic g10`}>
            <Box className={`flex jcsb aic g20`}>
              <Title
                ...
                fw={"500"}
              />
              <Typography variant="h6" className={`fw700`}>
                EGP {cartPrice}
              </Typography>
            </Box>
            <Box className={`flex jcsb aic g20`}>
              <Title title={`Delivery Fees`} h={"h6"} fw={"500"} />
              <Typography variant="h6" className={`fw700`}>
                EGP {Math.round(cartPrice * (5 / 100))}
              </Typography>
            </Box>
            <Box className={`flex jcsb aic g20`}>
              <Title title={`Total`} h={"h6"} fw={"700"} />
              <Typography variant="h5" className={`fw700`}>
                EGP {Math.round(cartPrice * (105 / 100))}
              </Typography>
            </Box>
          </Box>
        </Paper>
        <Box className={`flex jcsb aic g20`}>
          <SecondaryButton onClick={handleCloseConfirmOrderModal}>
            Cancel
          </SecondaryButton>
